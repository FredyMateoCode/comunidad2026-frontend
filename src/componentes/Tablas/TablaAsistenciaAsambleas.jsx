...
export default function TablaAsistenciaAsamblea({ datos = [], cargando = false, onToggleAsistencia }) {
  if (cargando) {
    return (
    ...
    return (
      <Paper sx={{ p: 3, textAlign: 'center', width: '100%' }}>
        <Typography variant="body1" color="text.secondary">
          No hay registros de asistencia para esta asamblea.
        </Typography>
      </Paper>
    );
  }
  ...
  return (
    <TableContainer component={Paper} sx={{ width: '100%' }}>
      <Table size="small" aria-label="tabla asistencias">
        ...
        <TableBody>
          {datos.map((row) => (
            <TableRow key={row.id_asis_asamblea} hover>
              <TableCell>{row.id_asis_asamblea}</TableCell>
              ...
              <TableCell align="center">
                <Switch
                  ...
                  onChange={() => onToggleAsistencia && onToggleAsistencia(row.id_asis_asamblea)}
                  color="primary"
                  size="small"
                />
              </TableCell>
